...
(function () {
  ...
  function mount(host) {
    ...
    host.innerHTML =
      '<div class="read-head">' +
        ...
        '<div class="read-tabs">' +
          '<button class="read-tab active" data-cat="全部">全部</button>' +
          '<button class="read-tab" data-cat="科技">科技</button>' +
          '<button class="read-tab" data-cat="财经">财经</button>' +
          '<button class="read-tab" data-cat="社会">社会</button>' +
          '<button class="read-tab" data-cat="体育">体育</button>' +
          '<button class="read-tab" data-cat="生活">生活</button>' +
          '<button class="read-tab" data-cat="娱乐">娱乐</button>' +
        '</div>' +
      '</div>' +
    ...
    function render() {
      var list = curCat === '全部' ? FEED : FEED.filter(function (f) { return f.cat === curCat; });
      feed.innerHTML = list.map(function (f, i) {
        ...
        return '<div class="read-card" data-idx="' + i + '">' +
          ...
          '<div class="read-card-foot">' +
            ...
            '<span class="read-card-read">' + esc(f.read) + '阅读</span>' +
    ...
    host.querySelectorAll('.read-tab').forEach(function (tab) {
      tab.onclick = function () {
        host.querySelectorAll('.read-tab').forEach(function (t) { t.classList.remove('active'); });
  ...
  function openArticle(a) {
    ...
    box.innerHTML =
      ...
      '<div class="read-article-meta">' + esc(a.src) + ' · ' + esc(a.time) + ' · ' + esc(a.read) + '阅读</div>' +
      '<div class="read-article-body">' + esc(a.summary) + '</div>' +
      '<div class="read-article-body" style="color:#888;margin-top:12px">这是 SecureChat 看一看频道精选资讯演示内容。更多精彩内容正在路上。</div>';
