...
function applyLoginMode() {
  ...
  $('username').placeholder = (showReg || useCode) ? t('username', '用户�?) : t('usernameOrEmail', '用户名或邮箱');
...
async function openQrLogin() {
  try {
    ...
    mask.innerHTML = '<div class="qr-login-card"><h3>扫码登录</h3><img alt="登录二维�?><p>请使用已登录�?SecureChat 手机端「扫一扫」扫描此二维码，2 分钟内有�?/p><button type="button">关闭</button></div>';
  ...
  } catch (e) { toast(e.message || '二维码生成失�?, 'error'); }
}
...
async function openQrAuth() {
  try {
    ...
    const mask = document.createElement('div');
    mask.className = 'qr-login-mask';
    mask.innerHTML = '<div class="qr-login-card"><h3>扫码登录授权</h3><img alt="登录二维�?><p>请在未登录的 SecureChat 客户端“扫码登录”扫描此二维码，2 分钟内有�?/p><button type="button">关闭</button></div>';
    mask.querySelector('img').src = state.serverHost + '/api/login/qr/image?token=' + encodeURIComponent(data.token);
    mask.querySelector('button').onclick = () => { if (qrLoginTimer) clearInterval(qrLoginTimer); mask.remove(); };
    document.body.appendChild(mask);
    let done = false;
...
// 每次（登�?进入聊天页）自动�?/api/version，若 latest > 当前 PACKAGE_VERSION 则弹更新浮层�?
...
document.querySelectorAll('.side-tab').forEach(tt => {
  tt.onclick = () => {
    ...
    if (tt.dataset.side === 'ai') {
      ...
      // 调用 ai.js 里的 switchToAi：它负责未配�?apiKey 时弹设置、聚焦输�?
      if (window.switchToAi) window.switchToAi();
      loadMiniPrograms();
      // 移动端：切到 AI 视图也要进入"聊天�?（AI 视图�?.main 平级，靠 mobile-chat-active 移入屏内�?
